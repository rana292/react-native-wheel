    ...
    if(this.state.childAngles.length == 0 && this.state.level2ChildAngles.length == 0) {
      return(
          ...
          <G width={width / 2} height={height / 2}>
            <Circle cx="0" cy="0" r={this.state.abortAnimation && !this.state.noChild ? "25" : "40"} fill="#0256E1"
            >
            </Circle>  
            <Text fontSize="8" x={0} y={this.props.semiCircle?-4:-4} textAnchor="middle" fill='#fff' NumberProp="12">
              {this.props.homeTitle}
            </Text>
            <Text fontSize="8" fontWeight='bold' x={0}  y={this.props.semiCircle?-4:8} fill='#fff' textAnchor="middle" >
              {this.props.rana}
            </Text>  
            <Text fontSize="5"  x={0} y={this.props.semiCircle?-4:17} fill='#fff' textAnchor="middle" style={this.props.style}>
              {this.props.txt}
            </Text>
    ...
    if(this.state.level2ChildAngles.length > 1) {
      ...
      return (
          <View style= {styles.mainView}>
            <Text numberOfLines= {3} style= {{alignSelf: 'center', fontSize: 15}}>
...
const styles = StyleSheet.create({
  mainView: {
    ...
    alignItems: 'center', 
    justifyContent: 'center',backgroundColor:'#f8f8ff',width:430,height:400
  },
  semiMainView: {
    flex: 1, 
    alignItems: 'center', 
